...
**核心特性**:
- 三玩法统一入口（同一大厅、同一登录）
- 白名单准入（SQLite + admin 在线管理，`allowed_users.json` 仅首次迁移种子）
- 精美 UI（赌场绿桌 + 暗金质感 + 微动效）
- 可选人机（房主可加入 AI 补位）
...
| **M4** | 重连 + 超时逻辑 | 动效 + 断线提示 | ✅ 已完成 |
| **M5** | Bot 调优 + 部署 | 构建 + 联调 | ✅ 已完成 |

**当前阶段**: 多玩法基础（上表 M0-M5）全部完成。其后又交付了一批产品力需求 #008~#014（白名单入库+admin、邀请/快速匹配、摊牌+聊天、积分榜、移动端+音效、牌局回放、UI 打磨），全部已 push origin/main。下一批规划见 `docs/internal/roadmap-2026Q3.md`（本地不入库）。注意：roadmap 里的 "M1~M5" 是这批需求的迭代编号，与上表的多玩法构建里程碑不是同一套，勿混。
...
**示例**: 契约修复类任务的写法——在 `docs/internal/` 下建 `[问题名]-TASK.md`，列出根因、改动点、验收标准，再喂给工程 agent。
...
**教训**: 阶段经验教训记录在 docs/internal/ 的阶段总结里(本地不入库,按需查阅)
...
3. 更新 `docs/internal/roadmap-2026Q3.md` 或里程碑报告
...
2. `docs/internal/roadmap-2026Q3.md` - 了解当前排期与已交付进度（本地不入库）
3. `git log -5 --oneline` - 了解最近提交

### 7.2 优先处理

- 检查 `docs/internal/roadmap-2026Q3.md` 是否有待分配任务
- 查看用户是否有新需求反馈
- 验证最近完成的功能是否需要验收
